...
data1 <- data1[!is.na(data1$PValue), ]
...
data1$differential <- ifelse(data1$PValue < 0.05, "significant", "non-significant")
# add adjusted pvalues
data1$log10pvalue <- -log10(data1$PValue)

print(head(data1))


volcano1 <- ggplot(data1, aes(x = logFC, y = log10pvalue, color = differential)) +
    geom_point(alpha = 0.8, size = 2) +
    scale_color_manual(values = c("significant" = "red", "non-significant" = "grey")) +
    theme_minimal() +
    labs(title = titlename, x = "logFC", y = "-log10(PValue)", color = "significance")

ggsave(finalname, plot = volcano1, width = 8, height = 6, dpi = 300, bg = "white")
#print(volcano1)

# now the MA plot part
ma1 <- ggplot(data1, aes(x = logCPM, y = logFC, color = differential)) +
    geom_point(alpha = 0.8, size = 2) +
    scale_color_manual(values = c("significant" = "red", "non-significant" = "grey")) +
    theme_minimal() +
